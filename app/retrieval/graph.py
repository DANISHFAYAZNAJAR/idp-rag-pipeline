...
from app.retrieval.bm25_cache import bm25_cache
...
async def node_hybrid_retrieve(state: RAGState) -> dict:
    ...
    bm25_retriever = bm25_cache.get(state["document_ids"], state["db_chunks"])
...
NO_ANSWER_MESSAGE = (
    "I couldn't generate an answer from the selected document(s).\n\n"
    "**Try:** pick a different document, re-upload the file, turn off streaming, "
    "or ask a more specific question about content in the document."
)
...
async def node_generate_answer(state: RAGState) -> dict:
    ...
    answer = response.content or NO_ANSWER_MESSAGE
